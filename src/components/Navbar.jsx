...
import { Search, ShoppingBag, User, Menu, Heart, X, ChevronRight, TrendingUp } from 'lucide-react';
    ...
    <>
      <div className="sticky top-0 z-50 bg-white group">
        ...
        <nav className="bg-white border-b border-gray-100 relative z-50">
                ...
                >
                  ...
                  {activeMenu === item && megaMenuData[item] && !isSearchOpen && (
                    <div className="fixed top-[115px] left-0 w-full bg-white shadow-xl border-t border-gray-100 py-12 animate-fade-in cursor-default" style={{zIndex: 60}}>
                      {/* ... (Mega Menu Content - exactly as you had it) ... */}
          ...
          <div 
            className={`absolute top-full left-0 w-full bg-white shadow-[0_20px_40px_rgba(0,0,0,0.08)] transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] origin-top z-40 border-t border-gray-100 ${
              isSearchOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-4 pointer-events-none absolute -z-10'
            }`}
          >
            <div className="max-w-[800px] mx-auto px-4 py-8 md:py-12">
              <form onSubmit={handleSearchSubmit} className="w-full relative">
                
                {/* Styled Input based on the provided reference image */}
              ...
              {/* Premium "Trending Searches" Quick Links */}
              <div className="mt-8">
        ...
        {/* Desktop Mega Menu Overlay (Hidden if search is open) */}
      ...
      {/* Clicking anywhere on this darkened background closes the search */}
      <div 
        className={`fixed inset-0 bg-black/40 backdrop-blur-[2px] z-40 transition-opacity duration-400 lg:top-[115px] top-[90px] ${
      ...
      <div className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] lg:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMobileMenuOpen(false)}>
         ...
         {/* Omitted here for brevity, keep your existing mobile menu drawer code */}
      </div>
    </>
  );
};
